...
$invsyid= $_GET['invsyid'];
 ?>
    ...
    <form action="assettransactionform.php?invsyid=<?php echo $invsyid ?>" method="post">
    ...
    <thead>
      ...
      <tbody>
      <h6>
            <input type="submit" class="btn btn-success" value="Asset Transaction Form">
            </h6>

    <?php
                            $sql = mysql_query("SELECT * FROM `inventory` JOIN `schoolbldg` JOIN `invlocation` WHERE schoolbldg.schoolbldgid = inventory.schoolbldgid AND invlocation.locationid = inventory.locationid AND invsyid = $invsyid") or die(mysql_error());
    ...
    while($row = mysql_fetch_array($sql)){
      ...
      <tr>
      <td><input type="checkbox" name="selector[]" value="<?php echo $assetid; ?>"></td>
            </form>
        ...
        <a href ="editasset.php?assetid=<?php echo $assetid; ?>"><i class="fa fa-pencil-o"> Edit</i></a></td>
      </tr>
<div id="editModal<?php echo $num; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
 <div class="modal-dialog" role="document">
    <div class="modal-content">
              ...
              <p class="lead"><?php
               echo $remarks;?></p>
                    ...
                    <a href="#updateModal<?php echo$num?>" data-sfid='"<?php echo $row['assetid'];?>"' data-toggle="modal" class="btn btn-primary">Update Asset</a>
    ...
    <div class="modal-dialog" role=document>
      ...
      <div id="updateModal<?php echo $num; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role=document>
        <div class="modal-content">
            <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
             ...
             <?php $updateinvid= $_GET['invsyid']; ?>
              ...
              <input type="text" class="form-control" name="priceupdate" id="priceupdate" onkeyup="calculate()" value="<?php echo $purchaseprice; ?>">
              </div>
              <div class="form-group">
              <label>Lot/Qty</label>
              <input type="text" class="form-control" name="qtyupdate" id="qtyupdate" onkeyup="calculate()" value="<?php echo $assetqty; ?>">
              </div>
              <div class="form-group">
              <label>Total</label>
              <input type="text" class="form-control" id="totalcostupdate" name="totalcostupdate" value="<?php echo $assettotal; ?>">
              </div>
              <div class="form-group">
              <label>Remarks</label>
              ...
              <div class="table-responsive">
                <table class="table">
                  <tr>
                    ...
                    <td><?php 
                    $abcd = mysql_query("SELECT ROUND(SUM(assettotal)) as total from inventory WHERE invsyid=$invsyid");
...
if(isset($_POST['delconfirm'])){
 $del = mysql_query("DELETE FROM inventory WHERE assetid = '$assetid'") or die(mysql_error());      
 if($del)
          {

        echo "<script type='text/javascript'>alert('Item deleted Successfully.'); window.location.href = 'invsystem.php?invsyid=$invsyid';</script>";
