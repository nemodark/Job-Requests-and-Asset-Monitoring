      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Asset Disposal Form</li>
          </ol>
        </section>
              ...
              <div class="box box-primary">
               <form role="form" name="transferform" method="post" action="">
                ...
                <label>Disposition:</label>
                      <select class="form-control" name="disposetype" id="disposetype" onchange="calculate()"  required>
                      <option>----</option>
                            <?php

                              $saql ="SELECT * FROM dispositiontype";
                              $result = mysql_query($saql);
                              while($row = mysql_fetch_array($result)){
                              extract($row);

                              echo "<option value=".$dispositiontypeid.">".ucfirst($dispositiontype)."</option>";
                            }
          
                            ?>
                         </select>
                </div>
                <div class="form-group col-md-4" id="gogopower">
                </div>
                <div class="form-group col-md-4" id="completevalue">
                </div>
                </div>
                </div>
                </CENTER>
                ...
                <div class="form-group col-md-4">
                <label>Teller Initial</label>
                <input type="text" class="form-control" name="tellerinitial" id="tellerinitial">
                </div>
                <div class="form-group col-md-4">
...
$(document).ready(function(){
  $('#disposetype').change(function(){
    var ofcid = $('#disposetype').val();
    if(ofcid != 0)
    {
      $.ajax({
        type:'post',
        url:'disposal.php',
        data:{dispositionid:ofcid},
        cache:false,
        success: function(returndata){
          $('#gogopower').html(returndata);
        }
      });
    }
  })
...
$(document).ready(function(){
  $('#disposetype').change(function(){
    var disposevalueid = $('#disposetype').val();
    if(disposevalueid != 0)
    {
      $.ajax({
        type:'post',
        url:'disposalnew.php',
        data:{dispositionvalueid:disposevalueid},
        cache:false,
        success: function(returndata){
          $('#completevalue').html(returndata);
        }
      });
    }
  })
